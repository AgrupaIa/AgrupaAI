...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let centroids = tf.randomUniform([clusters, 1]);
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        const distances = tf.sqrt(
            tf.sum(
                tf.square(
                    tf.sub(tensorData.expandDims(1), centroids.expandDims(0))
                ), 
                2
            )
        );
        ...
        for (let c = 0; c < clusters; c++) {
            ...
            const newCentroid = tf.mean(filteredData, 0);
    ...
    // Paso 1: Seleccionar columna numérica (ej: 'compras', 'total', etc.)
    const numericColumn = prompt("Ingresa el nombre de la columna numérica a analizar (ej: 'ventas', 'precio'):");
    if (!numericColumn || !processedData[0].hasOwnProperty(numericColumn)) {
        alert(`Columna "${numericColumn}" no encontrada. Asegúrate de escribir el nombre correcto.`);
        return;
    }

    // Paso 2: Extraer y normalizar datos
    const rawData = processedData.map(row => parseFloat(row[numericColumn]) || 0);
    const normalizedData = normalizeData(rawData);

    // Paso 3: Aplicar K-Means (3 grupos)
    try {
        const groups = await applyKMeans(normalizedData, 3);

        // Paso 4: Añadir grupos a los datos
        processedData.forEach((row, index) => {
            row.Grupo = groups[index]; // Nueva columna "Grupo"
        });

        // Paso 5: Actualizar la tabla y gráficos
        populateFilterOptions(); // Actualizar filtros con la nueva columna
        renderTableData();
        createBarCharts();
        
        alert(`¡Datos clasificados en 3 grupos! Columna añadida: "Grupo"`);
    } catch (error) {
        console.error("Error en K-Means:", error);
        alert("Error al clasificar los datos. Verifica la consola.");
    }
});
});
